...
3. Multi-scale features extracted from the original image
4. Specialized modules (PFMM, CFFM, SACM) to effectively integrate conditioning information
5. Wavelet-space Transformer Query 来自 Diffusion Noise, Key/Value 来自 Condition
...
class ImageFeatureEncoder(nn.Module):
    """
    简化的图像特征提取器，基于 ResNetV2，移除尺寸限制。
    提取 4 个尺度的特征用于 CFFM 和 SACM。
    """
    ...
    def __init__(self, width_factor=1):
        ...
        # Stage 1: 输出 width*4 = 256 channels
        ...
        # Stage 2: 输出 width*8 = 512 channels, stride=2
        ...
        # Stage 3: 输出 width*16 = 1024 channels, stride=2
    ...
    def forward(self, x):
        ...
        # Root:  H/2
        x = self.root(x)
        features.append(x)  # feat0: 64 channels, H/2
        ...
        # Stage 1: H/4
        x = self. stage1(x)
        features.append(x)  # feat1: 256 channels, H/4
        
        # Stage 2: H/8
        x = self.stage2(x)
        features.append(x)  # feat2: 512 channels, H/8
        
        # Stage 3: H/16
        x = self.stage3(x)
        features.append(x)  # feat3: 1024 channels, H/16
        
        return features


class TimeAdaptiveSubbandGating(nn.Module):
    """
    替代原有的 NBP-Filter。
    根据时间步 t，动态学习如何对 Low-Freq (LL) 和 High-Freq (LH, HL, HH) 进行加权。
    """
    def __init__(self, time_emb_dim, channels):
        super().__init__()
        # 输入时间嵌入，输出 4 个子带的门控权重
        # 4 * channels 表示为每个通道、每个子带都学习一个权重
        self.mlp = nn.Sequential(
    ...
    def forward(self, x_ll, x_high, time_emb):
        """
        x_ll: [B, C, H, W]
        x_high: [B, C, 3, H, W]  (包含 LH, HL, HH)
        time_emb: [B, time_dim]
        """
        B, C, _, _ = x_ll.shape
        
        # 计算门控权重 [B, C*4] -> [B, C, 4]
        gates = self.mlp(time_emb).view(B, C, 4)
        
        # 分离权重
        g_ll = gates[:, :, 0].view(B, C, 1, 1)        # Low-Freq 权重
        g_lh = gates[:, :, 1].view(B, C, 1, 1, 1)     # High-Freq 权重
        g_hl = gates[:, :, 2].view(B, C, 1, 1, 1)
        g_hh = gates[:, :, 3].view(B, C, 1, 1, 1)
        
        # 1. 对低频 LL 进行加权
        out_ll = x_ll * g_ll
        
        # 2. 对高频 High 进行加权 (需要拆解 x_high)
        # x_high 的维度通常是 [B, C, 3, H, W]
        h_lh = x_high[:, :, 0] * g_lh.squeeze(2)
        h_hl = x_high[:, :, 1] * g_hl.squeeze(2)
        h_hh = x_high[:, :, 2] * g_hh.squeeze(2)
        
        # 重新堆叠高频部分
        out_high = torch.stack([h_lh, h_hl, h_hh], dim=2)
        
        return out_ll, out_high

class WaveletCrossAttention(nn.Module):
    """
    在小波域进行的交叉注意力。
    Query 来自 Diffusion Noise, Key/Value 来自 Condition。
    """
    ...
    def forward(self, x, context):
        """
        x (Query): [B, C, H, W] - 扩散特征的某个子带
        context (Key/Value): [B, C, H, W] - 条件特征的对应子带
        """
        B, C, H, W = x.shape
        
        # 投影并变形为 [B, Heads, H*W, C/Heads]
        q = self.to_q(x).view(B, self.num_heads, -1, (H * W)).permute(0, 1, 3, 2)
        k = self.to_k(context).view(B, self.num_heads, -1, (H * W))
        v = self.to_v(context).view(B, self.num_heads, -1, (H * W)).permute(0, 1, 3, 2)

        # Attention 计算: Softmax(Q @ K^T / scale)
        attn = (q @ k) * self.scale
        ...
        # 最终投影 + 残差连接 (残差在外部做也可以，这里做投影)
        return self.proj(out)

class WS_Former(nn.Module):
    """
    [Main Module] Wavelet-Space Transformer
    替代 SS-Former。使用 DWT 分解特征，进行子带对齐和时间门控，再重建。
    """
    def __init__(self, in_channels, time_emb_dim, wave='haar', j_levels=1):
        super().__init__()
        
        # 1. 小波变换定义
        # J=1 表示做一次分解，得到 LL 和 (LH, HL, HH)
        self.dwt = DWTForward(J=j_levels, mode='zero', wave=wave)
        self.idwt = DWTInverse(mode='zero', wave=wave)
        
        # 2. 交叉注意力模块
        # LL 子带包含主要语义，非常重要
        self.attn_ll = WaveletCrossAttention(in_channels)
        
        # High 子带处理：为了节省计算量，我们将 3 个高频子带 (LH, HL, HH) 
        # 在通道维度合并后一起做 Attention，或者共享参数
        # 这里选择简单策略：对 3 个高频子带共享同一个 Attention 模块
        self.attn_high = WaveletCrossAttention(in_channels)
        
        # 3. 时间自适应门控 (核心创新点: 替代 Band-Pass Filter)
        self.gating = TimeAdaptiveSubbandGating(time_emb_dim, in_channels)
    ...
    def forward(self, noise_feat, cond_feat, time_emb):
        """
        noise_feat: 来自 Diffusion Decoder 的特征 [B, C, H, W]
        cond_feat: 来自 Condition Encoder 的特征 [B, C, H, W]
        time_emb: 时间步嵌入 [B, dim]
        """
        # --- 1. 小波分解 (DWT) ---
        # Yl: 低频 [B, C, H/2, W/2]
        # Yh: 高频列表，第一项为 [B, C, 3, H/2, W/2]
        n_ll, n_high = self.dwt(noise_feat)
        c_ll, c_high = self.dwt(cond_feat)
        
        # 取出第一层的高频部分
        n_h = n_high[0] # [B, C, 3, H, W]
        c_h = c_high[0]
        
        # --- 2. 小波域交叉注意力 (Interaction) ---
        
        # A. 低频部分对齐 (语义对齐)
        # 这里的注意力利用了空间信息 (因为 DWT 保留了空间对应关系)
        refined_n_ll = self.attn_ll(n_ll, c_ll) + n_ll # 残差连接
        
        # B. 高频部分对齐 (边缘/细节对齐)
        # 将 3 个高频方向 (LH, HL, HH) 视为 Batch 的一部分或者独立处理
        # 这里通过 reshape 将 3 合并到 Batch 维度进行并行计算
        B, C, Sub, H, W = n_h.shape
        n_h_flat = n_h.view(B * 3, C, H, W)
        c_h_flat = c_h.view(B * 3, C, H, W)
        
        refined_n_h_flat = self.attn_high(n_h_flat, c_h_flat) + n_h_flat
        
        # 还原形状
        refined_n_h = refined_n_h_flat.view(B, C, 3, H, W)
        
        # --- 3. 时间自适应门控 (Time-Adaptive Filtering) ---
        # 根据当前扩散步数 t，决定保留多少低频语义 vs 高频纹理
        gated_ll, gated_h = self.gating(refined_n_ll, refined_n_h, time_emb)
        
        # --- 4. 小波重构 (IDWT) ---
        # idwt 需要的输入格式 Yh 是一个列表
        out = self.idwt((gated_ll, [gated_h]))
...
#     # 注意：pytorch_wavelets 在 CPU 上可能较慢或报错，建议在 GPU 运行
#     if device == 'cuda':
#         model = WS_Former(in_channels=C, time_emb_dim=time_dim).to(device)
#         x = torch.randn(B, C, H, W).to(device)
#         cond = torch.randn(B, C, H, W).to(device)
#         t_emb = torch.randn(B, time_dim).to(device)
        
#         output = model(x, cond, t_emb)
#         print(f"Input shape: {x.shape}")
#         print(f"Output shape: {output.shape}") # 应该与输入一致
#     else:
#         print("请在 CUDA 环境下测试以确保 pytorch_wavelets 正常工作")
...
class ConditionalDenoisingUNet(nn.Module):
    ...
    def __init__(
        ...
        # 输入只有 noisy mask
        total_in_channels = actual_in_channels
        ...
        actual_coarse_channels = num_classes if num_classes > 2 else 1  # 二分类时为 1
        self.pfmm = PFMM(in_channels=base_channels, num_classes=actual_coarse_channels)
        # PFMM:  用 coarse_mask 调制特征
        # self. pfmm = PFMM(in_channels=base_channels, num_classes=num_classes)
        
        # ResNet for extracting image features
        self.image_encoder = ImageFeatureEncoder(width_factor=1)
        
        # Encoder 各层输出通道数
        self.enc_channels = [base_channels * m for m in channel_mult]
        # enc_channels = [64, 128, 256, 512]
        
        # ResNet feature channels (width=64)
        # feat0: 64, feat1: 256, feat2: 512, feat3: 1024
        resnet_feature_channels = [64, 256, 512, 1024]
        
        # ========== 添加通道对齐投影层 ==========
        # 将 ResNet 特征通道数投影到与 encoder 对应层相同的通道数
        self.feat_projections = nn.ModuleList()
        for i in range(4):
            if resnet_feature_channels[i] != self.enc_channels[i]: 
                # 需要投影
                self.feat_projections. append(
                    nn.Sequential(
                        nn.Conv2d(resnet_feature_channels[i], self.enc_channels[i], 1, bias=False),
                        nn. GroupNorm(8, self.enc_channels[i]),
                        nn.ReLU(inplace=True)
                    )
                )
            else:
        ...
        # CFFM modules for encoder levels 0, 1, 2
        # 现在 x_channels 与 f_channels 相同（经过投影后）
        self.cffms = nn.ModuleList([
            CFFM(f_channels=self.enc_channels[i], x_channels=self.enc_channels[i])
            for i in range(3)
        ])
        
        # SACM module for encoder level 3
        # 同样，x_channels 经过投影后与 s_channels 匹配
        self.sacm = SACM(
            x_channels=self.enc_channels[3],  # 投影后为 512
            s_channels=self. enc_channels[3]   # 512
        )
        
        # WS-Former module for encoder level 3 (Replacing SACM)
        # Using ResNet high-level features as condition
        self.ws_former = WS_Former(
    ...
    def forward(
        ...
        # ========== 提取图像多尺度特征（直接返回列表） ==========
        image_features = self.image_encoder(image_cond)
        # image_features = [feat0, feat1, feat2, feat3]，无需反转
        
        # ========== 对 ResNet 特征进行通道投影 ==========
        projected_features = []
        for i, feat in enumerate(image_features):
            projected_features.append(self.feat_projections[i](feat))
        
        # Initial convolution
        h = self. init_conv(x)
        
        # PFMM:  用 coarse_mask 调制特征
        h = self.pfmm(h, coarse_mask_cond)
        ...
        for level_idx in range(len(self.channel_mult)):
            ...
            # ========== CFFM / SACM 融合图像特征 ==========
            img_feat = projected_features[level_idx]  # 已经投影过的特征
            
            # 确保空间尺寸匹配
            if img_feat.shape[2:] != h.shape[2:]: 
            ...
            if level_idx < 3:
                # 前三层: CFFM 融合
                h = self.cffms[level_idx](h, img_feat)
                # h = h
            else:
                # 最后一层:  WS_Former 融合 (替代 SACM)
                h = self.sacm(h, img_feat)
                h = self.ws_former(h, img_feat, t_emb)
